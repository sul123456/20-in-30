...
function rowData(a) {
  ...
  return { sno: row.sno || "—", feature: row.feature || "—", month: row.month || "—", stage: row.stage || "", status: row.status || "" };
}
...
export default function AdminAudit({ user, audit, loading, error, reload }) {
  ...
  const rows = useMemo(() => {
    ...
    return (audit || []).filter((a) => {
      ...
      if (!q) return true;
      const d = rowData(a);
      return [actor(a), a.actor_email, a.operation, d.sno, d.feature, d.stage, d.status, d.month, a.table_name]
    ...
    <div className="dash">
      ...
      <section className="filters">
        ...
        <div className="filters-grid audit-filters">
          ...
          <div className="field"><label>Action</label><select className="input" value={op} onChange={(e)=>setOp(e.target.value)}><option value="">All actions</option><option value="INSERT">INSERT</option><option value="UPDATE">UPDATE</option><option value="DELETE">DELETE</option><option value="HISTORY">Historical status</option></select></div>
      ...
      <section className="panel">
        ...
        {loading ? <div className="loading">Loading audit trail…</div> : error ? <div className="empty-state"><b>Couldn’t load audit trail</b>{error}</div> : rows.length === 0 ? (
          ...
          <div className="scroll-x"><table className="audit-tbl"><thead><tr><th>Time</th><th>Action</th><th>Person</th><th>S.No.</th><th>Feature</th><th>Stage / Status</th><th>Month</th><th>Record</th></tr></thead>
            <tbody>{rows.map((a) => { const d=rowData(a); return <tr key={a.id}><td className="num">{new Date(a.changed_at).toLocaleString("en-IN",{dateStyle:"medium",timeStyle:"short"})}</td><td><span className={"audit-op audit-"+String(a.operation).toLowerCase()}>{a.operation}</span></td><td><b>{actor(a)}</b>{a.actor_email && <small>{a.actor_email}</small>}</td><td className="num">{d.sno}</td><td>{d.feature}</td><td>{d.stage || "—"}{d.status ? ` → ${d.status}` : ""}</td><td>{d.month}</td><td className="audit-id">{a.row_id || "—"}</td></tr>; })}</tbody>
          </table></div>
        )}
      </section>
    </div>
